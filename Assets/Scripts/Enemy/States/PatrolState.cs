...
namespace BHSCamp
{
    public class PatrolState : FsmState
    {
        ...
        private void CheckForPlayer()
        {
            ...
            if (_enemy is EnemyWithAttack attackEnemy && attackEnemy.PlayerInSight()) {
                Fsm.SetState<AttackState>();
            }

            // Question:
            // Изначально код был таким:
            // if (_enemy is EnemyWithAttack attackEnemy) {
            //     Fsm.SetState<AttackState>();
            // }
            // Однако, противник не ходил и просто стоял на месте. Не могу понять почему..
